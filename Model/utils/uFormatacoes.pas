unit uFormatacoes;
interface
uses System.SysUtils, Vcl.StdCtrls, Classes, Vcl.Mask ;

type
    TFormato = (CNPJ, CPF, InscricaoEstadual, CNPJorCPF, TelefoneFixo, Celular, Personalizado,
                Valor, Money, CEP, Dt, Peso, estoque);
...
function FormatarEstoque(str : string): string;
begin
 if Str.IsEmpty then
        Str := '0';
...
begin
    ...
    begin
        ...
        if Formato= estoque then
             texto:=FormatarEstoque(SomenteNumero(texto)) ;
        if obj is TEdit then
        begin
